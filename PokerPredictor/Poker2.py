...
def plot_all_ai_learning_progress(ai_players):
    ...
    # First subplot: Balance + Exploration
    for ai in ai_players:
        rounds = [entry["round"] for entry in ai.learning_log if entry["balance"] is not None]
        balances = [entry["balance"] for entry in ai.learning_log if entry["balance"] is not None]
        rewards = [entry["reward"] for entry in ai.learning_log if entry["reward"] is not None]
        #exploration = [entry["exploration"] for entry in ai.learning_log if entry["exploration"] is not None]

        axs[0].plot(rounds, balances, label=f"{ai.name} Balance")
        #axs[0].plot(rounds, exploration, label=f"{ai.name} Exploration", linestyle="dashed")

    axs[0].set_ylabel("Balance / Exploration")
    axs[0].legend()
    axs[0].set_title("AI Balance and Exploration Over Time")

    # Second subplot: Reward
    for ai in ai_players:
        rounds = [entry["round"] for entry in ai.learning_log]
        rewards = [entry["reward"] for entry in ai.learning_log]
        axs[1].plot(rounds, rewards, label=f"{ai.name} Reward")

    axs[1].set_ylabel("Reward")
    axs[1].set_xlabel("Round")
    axs[1].legend()
    axs[1].set_title("AI Reward History")

    for ai in ai_players:
        for r in ai.reset_rounds:
            axs[0].axvline(x=r, color='gray', linestyle='--', alpha=0.4)
            axs[1].axvline(x=r, color='gray', linestyle='--', alpha=0.4)
